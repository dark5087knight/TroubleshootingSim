...
def handle_update(args):
    """
    Check if a newer version exists on GitHub and optionally update by
    cloning the repo and running the installer.
    """
    # Load local metadata
    ...
    try:
        ...
        if not install_script.exists():
            print("install script not found in repo!")
            sys.exit(1)
        ...
        subprocess.run([str(install_script)], cwd=tmp_dir, check=True)
